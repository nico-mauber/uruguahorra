...
export class XPService {
  ...
  static async awardXP(
    ...
    try {
      ...
      switch (eventType) {
        case 'contribution':
          xpEarned = calculateContributionXP(eventData.amount || 0);
          break;
  ...
  static async getTopXPUsers(limit: number = 10): Promise<Array<{
    ...
    try {
      ...
      if (error) {
        logger.error(LogModule.DB, 'Error obteniendo ranking de XP', error);
        throw error;
      }
      ...
      (data || []).forEach((entry: any) => {
        ...
        userXPMap[userId].total_xp += entry.xp_earned;
      });

      // Convertir a array y ordenar
      const topUsers = Object.values(userXPMap)
