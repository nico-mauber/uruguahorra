...
export interface UseStreakNotificationsReturn {
  ...
  updateSettings: (newSettings: Partial<StreakNotificationSettings>) => Promise<void>;
...
export function useStreakNotifications(): UseStreakNotificationsReturn {
  ...
  const loadSettings = async () => {
    try {
      ...
      logger.error(LogModule.API, 'Error cargando configuración de notificaciones', error);
    }
  };
  ...
  const saveSettings = async (newSettings: StreakNotificationSettings) => {
    try {
      ...
      logger.error(LogModule.API, 'Error guardando configuración de notificaciones', error);
    }
  };
  ...
  const initialize = useCallback(async (): Promise<boolean> => {
    try {
      ...
      logger.error(LogModule.API, 'Error inicializando notificaciones de racha', error);
      return false;
    }
  }, [settings.enabled]);
   ...
   */
  const setupDailyReminder = useCallback(async (hour: number, minute: number): Promise<boolean> => {
    try {
      if (!permissionsGranted) {
        logger.warn(LogModule.API, 'No hay permisos para programar notificación');
        return false;
      }
      ...
      logger.error(LogModule.API, 'Error configurando recordatorio diario', error);
      return false;
    }
  }, [permissionsGranted]);
   ...
   */
  const setupStreakWarning = useCallback(async (hoursBeforeBreak: number): Promise<boolean> => {
    try {
      ...
      const secondsUntilAlert = Math.floor((alertTime.getTime() - now.getTime()) / 1000);
      ...
      logger.error(LogModule.API, 'Error configurando alerta de racha', error);
      return false;
    }
   ...
   */
  const updateSettings = useCallback(async (newSettings: Partial<StreakNotificationSettings>): Promise<void> => {
  ...
  const updateScheduledCount = async () => {
    try {
      ...
      const streakNotifications = notifications.filter(
        notif => notif.content.data?.type?.includes('streak')
      );
  ...
  const sendTestNotification = useCallback(async (): Promise<void> => {
    if (!permissionsGranted) {
      logger.warn(LogModule.API, 'No hay permisos para enviar notificación de prueba');
      return;
    }
   ...
   */
  const sendTestStreakReminder = useCallback(async (delaySeconds: number = 10): Promise<void> => {
    if (!permissionsGranted) {
      logger.warn(LogModule.API, 'No hay permisos para enviar recordatorio de prueba');
      return;
    }

    await NotificationsService.scheduleTestStreakReminder(delaySeconds);
    await updateScheduledCount();
    logger.info(LogModule.API, 'Recordatorio de prueba programado', { delaySeconds });
  }, [permissionsGranted]);

  const sendTestStreakWarning = useCallback(async (delaySeconds: number = 15): Promise<void> => {
    if (!permissionsGranted) {
      logger.warn(LogModule.API, 'No hay permisos para enviar alerta de prueba');
      return;
    }

    await NotificationsService.scheduleTestStreakWarning(delaySeconds);
    await updateScheduledCount();
    logger.info(LogModule.API, 'Alerta de prueba programada', { delaySeconds });
  }, [permissionsGranted]);

  const startDevReminder = useCallback(async (intervalMinutes: number = 1): Promise<void> => {
    if (!permissionsGranted) {
      logger.warn(LogModule.API, 'No hay permisos para recordatorio de desarrollo');
      return;
    }

    await NotificationsService.scheduleDevReminder(intervalMinutes);
    await updateScheduledCount();
    logger.info(LogModule.API, 'Recordatorio de desarrollo iniciado', { intervalMinutes });
  }, [permissionsGranted]);
